...
function getSlips {
	...
	if [[ $avail != ?([0-9]) ]]
	then
		>&2 echo "WARNING: invalid slip days for $ID ($avail)"
		echo 0
		return 0
	fi
	...
	if [ -z "$result" ]
	then
		>&2 echo "WARNING: empty slip days used in $ID $readmefile ($result)"
		echo 0
		return 0
	fi
	if [[ $result != ?([0-9]) ]]
	then
		>&2 echo "WARNING: invalid slip days used in $ID $readmefile ($result)"
		echo 0
		return 0
	fi
