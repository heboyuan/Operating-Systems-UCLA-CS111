...
int main(int argc, char **argv){
  ...
  //====================================================//
  //part2 group + part3 free block + part4 free inode   //
  //====================================================//
  ...
  for(group_id = 0; group_id < num_group; group_id++){
    ...
    for(byte = 0; byte < block_size; byte ++){
      int buffer;
      pread(fd, &buffer, 1, cur_group.bg_block_bitmap*block_size + i)
      int bit;
      for(bit = 0; bit < 8; bit++){
        if((buffer & (1 << bit)) == 0){
    ...
    for(byte = 0; byte < block_size; byte ++){
      int buffer;
      pread(fd, &buffer, 1, cur_group.bg_inode_bitmap*block_size + i)
      int bit;
      for(bit = 0; bit < 8; bit++){
        if((buffer & (1 << bit)) == 0){
          fprintf(stdout, "BFREE,%d\n", byte*8+bit+1);
